...
public class TapewormController : MonoBehaviour
{
    [Header("Prefabs")]
    [SerializeField]
    ...
    void SegmentSetup()
    {
        ...
        for (int i = 0; i < numSegments; i++)
        {
            ...
            TapewormController mg = obj.GetComponent<TapewormController>();
    ...
    void FixedUpdate()
    {
        ...
        else
        {
            if (player == null)
            {
                player = target.GetComponent<TapewormController>().player;
            }
            if (target != null)
            {
                sp.sprite = segment;
